"""
Example usage of the enhanced Scheduler for ePIC EIC detector optimization.
"""

import numpy as np
import os
from ax.service.ax_client import AxClient
from scheduler import AxScheduler, JobLibRunner, JobType, Job
...
def main():
    ...
    runner = JobLibRunner(
        ...
        config={
            ...
            'container_engine': 'docker'  # For container jobs
        }
    )
    
    # Create the scheduler with enhanced options
    scheduler = AxScheduler(
        ax_client, 
        runner,
        config={
            'monitoring_interval': 5,  # Check every 5 seconds
            'job_output_dir': './detector_outputs',  # Directory for job outputs
            'synchronous': True,  # Run trials synchronously
            'cleanup_after_completion': True  # Clean up after trials
        }
    )
    ...
    # Alternatively, you could use a script or container:
    # scheduler.set_script_objective("./detector_simulation.py")
    # scheduler.set_container_objective("epic/detector-sim:latest", "python /app/simulate.py")
    ...
    scheduler.save_experiment("detector_optimization_results.json")
