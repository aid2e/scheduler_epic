...
class AxScheduler:
    ...
    def run_optimization(self, max_trials: int = 10) -> Dict[str, Any]:
        ...
        for _ in range(max_trials):
            ...
            if not self.synchronous:
                ...
                while not trial.check_status() in [TrialState.COMPLETED, TrialState.FAILED, TrialState.CANCELLED]:
                    time.sleep(self.monitoring_interval)
    ...
    def monitor_trials(self) -> None:
        ...
        for trial_index, trial in self.trials.items():
            ...
            if trial_state == TrialState.COMPLETED and trial_index in self.experiment.trials:
                ax_trial = self.experiment.trials[trial_index]
        ...
        if not self.scheduler.synchronous:
            ...
            while not all_done:
                ...
                for trial_index in trial_indices:
                    ...
                    if trial and trial.check_status() not in [TrialState.COMPLETED, TrialState.FAILED, TrialState.CANCELLED]:
                        all_done = False
                        break
